# WeeklyRateService class is responsible for adding new weekly rates.
class WeeklyRateService
  ...
  def missing_rate_dates(existing_currency_rates = [])
    ...
    @period.times do
      unless existing_currency_rates.include?(@start_date)
        missing_currency_rates.push(@start_date)
      end
      @start_date -= 1.week
    end
    missing_currency_rates
  ...
  def load_missing_rates(missing_currency_rates)
    base = base_currency
  ...
  def save_rates(currency_rates)
    ActiveRecord::Base.transaction do
      ...
      weekly_rate_records(currency_rates, currencies_list)
    end
  end

  def weekly_rate_records(currency_rates, currencies_list)
    currency_rates.each do |rates|
      ...
      rates['rates'].each do |code, rate|
        ...
        WeeklyRate.create!(rate: rate, rate_date: rate_date,
